                    ...
                    LazyVGrid(columns: goalItems, spacing: 10) {
                        ForEach(goalListViewModel.goals, id: \.id) { goal in
                            NavigationLink(destination: {
                                GoalDetailView(goalDetailViewModel: GoalDetailViewModel(repo: repo, goal: goal))
                            }) {
                                GoalView(goalViewModel: goal)
                                    .rotationEffect(.degrees(isEditing ? 2.5 : 0))
                                    .animation(Animation.easeInOut(duration: 0.13).repeat(while: isEditing), value: isEditing)
                                    .disabled(isEditing)
